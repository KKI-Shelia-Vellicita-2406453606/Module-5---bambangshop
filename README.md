...
1. In this tutorial, the system utilizes the Push model of the Observer pattern. This is characterized by the Publisher (the "Subject") actively sending state information to the subscribers whenever an event occurs. In the code you implemented, the main application initiates an HTTP POST request to the subscriber's URL, carrying a payload that contains all the relevant notification data. This ensures the subscriber receives the information immediately without having to ask for it.

2. If the system were to use the Pull model instead, the primary advantage would be that subscribers could control when they retrieve data, allowing them to manage their own resource consumption and pull only the specific data they need. This can be more robust if the publisher's state changes extremely rapidly. However, the disadvantage is increased latency and overhead; subscribers would need to "poll" the publisher frequently to check for updates, which can waste network bandwidth if there are no new changes, or cause them to miss updates if they don't poll often enough.

3. Removing multi-threading from the notification process would result in a significant performance bottleneck for the main application. Without the thread::spawn logic, the notify function would have to process every subscriber one by one in a sequence. Because each notification involves an external HTTP request, if one subscriber's server is slow or unresponsive, the entire main application would "hang" and wait for that request to timeout before moving to the next subscriber. This would make the shop feel unresponsive to the owner, as a simple task like deleting a product would not finish until all subscribers have been successfully notified.